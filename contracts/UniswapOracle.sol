...
contract UniswapOracle {
     ...
     * @param token The token address
     * @param amount Amount of tokens
     * @return price of USD (with 18 decimals) in tokens
     **/
    function usdToToken(address token, uint amount) external view returns (uint) {
        address uniswapPair = uniswapFactory.getPair(token, WETH);
        require(uniswapPair != address(0), "USDP: UNISWAP_PAIR_DOES_NOT_EXIST");
        ...
        return amount.mul(tokenReserve).mul(ethUsdDenom).div(wethReserve).div(ethUsdNum);
    }
