...
const DUMMY_SIGNATURE = `0x${web3.utils.padLeft(
	'',
	REAL_SIGNATURE_SIZE
)}`;
 ...
 */
const getSignerFor = (contractAddress, contract, signer) => (
	redeemer,
	methodName,
	methodArgs = []
) => {
	const parts = [contractAddress, redeemer];

	// if we have a method, add it to the parts that we're signing
	if (methodName) {
		methodArgs = methodArgs.map(arg => arg.toString());
